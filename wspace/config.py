...
def reload_config_for_win(win: Dependency('win')):
    ...
    win.widget.resize(1855, 1028)
    from PyQt4.QtCore import Qt
    win.layout.split([], Qt.Vertical)
    win.layout.get_widget([]).setSizes([976, 28])
    win.layout.split([0], Qt.Vertical)
    win.layout.get_widget([0]).setSizes([749, 221])
    win.layout.split([0, 0], Qt.Horizontal)
    win.layout.get_widget([0, 0]).setSizes([916, 915])
    ddic['actions/switch_view'](ddic['view/actions.py'], [0, 0, 0])
    ddic['actions/switch_view'](ddic['view/actions.py'], [0, 0, 1])
