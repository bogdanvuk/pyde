...
def reload_config_for_win(win: Dependency('win')):
    ...
    ddic.provide('view/trimac_receive_rtl.vhd', ddic['cls/view']('trimac_receive_rtl.vhd',win,file_name='/data/projects/pyde/pyde/trimac_receive_rtl.vhd'))
    win.widget.resize(1855, 1056)
    win.widget.showMaximized()
    from PyQt4.QtCore import Qt
    win.layout.split([], Qt.Vertical)
    win.layout.get_widget([]).setSizes([1004, 28])
    win.layout.split([0], Qt.Vertical)
    win.layout.get_widget([0]).setSizes([770, 228])
    win.layout.split([0, 0], Qt.Horizontal)
    win.layout.get_widget([0, 0]).setSizes([917, 914])
