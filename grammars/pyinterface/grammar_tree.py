...
class SemanticASTBuilder(ParseTreeVisitor):
    ...
    def build(self):
        ...
        for name, rule in self.ast.items():
            self.semantic_rule = SemanticRule(name)
            self.visit(rule)
...
#         pass
class SemanticNode:
    def __init__(self, features, parse_node, rule, parent = None):
        self.features = features
        self.parse_node = parse_node
        self.type = parse_node.type
        self.rule = rule
...
class SemanticTreeBuilder(ParseTreeVisitor):
    ...
    def visit(self, node):
        if hasattr(node, 'features') and node.features:
            parent = self.cur_parent
            semantic_node = SemanticNode(list(node.features.keys()), node, self.semantic_ast[node.type], parent)
