...
architecture_body
  : ARCHITECTURE name_=identifier OF entity_=identifier IS
    decl_=architecture_declarative_part
    BEGIN
    stmts_=architecture_statement_part
...
design_file
  : ( design_unit_ += design_unit )* EOF
  ;

design_unit
  : context_clause_ = context_clause library_unit_ = library_unit
  ;
...
library_clause
  : LIBRARY name_+=identifier ( COMMA name_+=identifier )* SEMI
  ;
...
use_clause
  : USE name_+=selected_name ( COMMA name_+=selected_name )* SEMI
  ;
